...
class Coord
  ...
  def copy
    Coord.new(@x, @y, @distance)
  end
  def up(dist)
    Coord.new(@x, @y + dist, @distance + dist)
  end
  def down(dist)
    Coord.new(@x, @y - dist, @distance + dist)
  end
  def left(dist)
    Coord.new(@x - dist, @y, @distance + dist)
  end
  def right(dist)
    Coord.new(@x + dist, @y, @distance + dist)
  end
...
puts "distance: #{input[0].reduce(0) do |memo, cmd| memo + cmd[1..-1].to_i end}"
...
# puts coords.size
# puts "max: #{coords.map(&:manhattan_distance).max}"
# puts "min: #{coords.map(&:manhattan_distance).min}"

pos = Coord.zero
coords2 = Set[Coord.zero]
input[1].each_with_index do |cmd, i|
  # print i
  dir = cmd[0]
  distance = cmd[1..-1].to_i
...
closest = intersections.reduce(Coord.new(Float::INFINITY, Float::INFINITY, 0)) do |closest, coord|
  if coord.manhattan_distance < closest.manhattan_distance && coord != Coord.zero
    coord
  else
    closest
  end
end
#293 is manhattan distance of closest (part 1)
puts "part 1: #{closest.manhattan_distance}"
puts closest.distance
...
# potential_closest_points = coords & coords2

closest2 = coords.reduce(Float::INFINITY) do |min, c|
  if coords2.include?(c)
    c2 = coords2.select { |cx| cx == c }.first
    combined_steps = c2.distance + c.distance
    if combined_steps < min && combined_steps != 0
      combined_steps
    else
      min
    end
